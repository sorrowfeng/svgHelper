...
const QList<QChar> kCmdkTypeList = {'M', 'L', 'H', 'V', 'C',
                                    'S', 'Q', 'T', 'A', 'Z'};
const QList<QChar> kSplitList = {' ', '-', ',', '\r', '\n', '\t'};
...
void SvgHelper::parseSvgImage(const QString& filepath) {
      ...
      while (!node.isNull())  //如果节点不空
      {
        if (node.isElement())  //如果节点是元素
        {
          ...
          if (kTypeList.contains(tagname)) {
            ...
            foreach (QString type, kTypeList) {
...
void SvgHelper::dealParsePainter(QPainterPath& path, QString line) {
  ...
  switch (kCmdkTypeList.indexOf(cmd.toUpper().at(0))) {
    case 0:  // m
    {
